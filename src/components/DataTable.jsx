...
const DataTable = ({
  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-white">
      ...
      <div
        className="md:hidden flex flex-col gap-2.5 p-2 sm:p-3 overflow-y-auto flex-1 bg-slate-50/50 scrollbar-hide"
      ...
      {!totalRow && (
        <div className="px-2.5 sm:px-4 py-2 border-t border-gray-200 bg-gray-50 flex items-center justify-between gap-2 sm:gap-4 rounded-b-lg flex-shrink-0">
          {/* Left Side: Row Dropdown */}
          <div className="flex items-center gap-1.5 sm:gap-2">
            <select
              value={itemsPerPage}
              onChange={(e) => onItemsPerPageChange(Number(e.target.value))}
              className="border border-gray-300 rounded-md px-1.5 sm:px-2 py-1 focus:outline-none focus:border-indigo-500 bg-white font-medium text-xs md:text-sm shadow-sm"
            >
          ...
          <div className="flex items-center gap-1.5 sm:gap-3 text-gray-700">
            <button
              ...
              className="p-1 sm:p-1.5 md:px-2 md:py-1 border border-gray-300 rounded-md bg-white disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50 transition shadow-sm flex items-center justify-center text-indigo-600"
            >
              <ChevronLeft size={16} strokeWidth={2.5} />
            </button>
            <div className="flex items-center text-xs md:text-sm font-semibold text-gray-600 whitespace-nowrap">
