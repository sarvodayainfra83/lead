...
import { PENDING_TABLE_HEADERS, formatDisplayDate } from './assignVisitorConstants';
...
const DATE_FILTER_OPTIONS = [
  { value: 'all', label: 'All Lead Dates' },
...
const parseDateObj = (item) => {
  const val = item?.visitDate || item?.nextCallDate || item?.nextDate || item?.leadDate || item?.timestamp || item?.date || item?.created_at;
  if (!val) return null;
...
export default function PendingAssignVisitor({ tabBar, onRefresh }) {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [pendingData, types, receivers] = await Promise.all([
        visitorApi.getPendingVisitorsWithLeads(),
        masterApi.getLeadTypes(),
        masterApi.getLeadReceivers()
      ]);

      const userPending = (pendingData || []).filter(l =>
        isUserAdmin(user) || matchesUserAssignment(l, user) || matchesUserReceiver(l, user)
      );
      setLeads(userPending);
      setLeadTypesMaster(types || []);

      const managerNames = Array.from(
        new Set((receivers || []).map(r => r.personName).filter(Boolean))
      );
      setRelationshipManagers(managerNames);
    } catch (err) {
      console.error('Failed to load pending visitors:', err);
  ...
  const renderRow = (item, idx) => (
    <tr key={item.id || item.leadNo || idx} className="group hover:bg-indigo-50/30 transition-colors border-b border-gray-100">
      ...
      >
        ...
        >
          <UserPlus size={12} /> Assign Visitor
        </button>
      </td>
      ...
      <td className={`px-4 py-3 text-center text-[13px] whitespace-nowrap ${NEXT_DATE_CLASS}`}>{formatDisplayDate(item.visitDate || item.nextCallDate || item.nextDate)}</td>
      ...
      <td className="px-4 py-3 text-left text-[13px] text-gray-600 whitespace-nowrap max-w-[200px] truncate" title={item.visitRemarks || item.customerSaid || ''}>{item.visitRemarks || item.customerSaid || '-'}</td>
    </tr>
  );
  ...
  const renderCard = (item) => (
    <div key={item.id || item.leadNo} className="bg-white rounded-lg border border-indigo-50 shadow-sm p-3 space-y-2">
      <div className="flex justify-between items-start border-b border-gray-100 pb-2">
        ...
        {(item.visitDate || item.nextCallDate || item.nextDate) && (
          <span className="text-[9px] px-2 py-0.5 rounded-full font-semibold uppercase border bg-indigo-50 text-indigo-700 border-indigo-200">
            Visit: {formatDisplayDate(item.visitDate || item.nextCallDate || item.nextDate)}
          </span>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 text-[10px]">
        ...
        {(item.visitRemarks || item.customerSaid) && (
          <div className="col-span-2">
            <p className="text-gray-400 uppercase tracking-tighter text-[8px]">Visit Remarks</p>
            <p className="text-gray-700 leading-tight">{item.visitRemarks || item.customerSaid}</p>
          </div>
        )}
      </div>
    ...
    <div className="p-0 sm:p-2 md:p-6 space-y-2 md:space-y-6 flex flex-col h-full min-h-0">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center gap-2 lg:gap-3 w-full px-2 sm:px-0">
        ...
        <div className={`${showMobileFilters ? 'grid' : 'hidden'} lg:hidden grid-cols-1 sm:grid-cols-3 gap-2 w-full`}>
          ...
          />
          <SearchableDropdown
            options={relationshipManagers.map(m => ({ value: m, label: m }))}
            value={filters.relationshipManager}
            onChange={(val) => { setFilters({ ...filters, relationshipManager: val }); setCurrentPage(1); }}
            placeholder="All Relationship Managers"
            height="h-[32px]"
          />
        </div>
