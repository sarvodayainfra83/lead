...
export default function HistoryVisitorFollowUp({ tabBar, onRefresh }) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const visitors = Array.from(
        new Set((userHistory || []).map(h => h.assignedVisitor).filter(Boolean))
      );
      setVisitorsList(visitors);
    } catch (err) {
      console.error('Failed to load history visitor follow-ups:', err);
  ...
  const filteredLeads = historyLeads.filter(l => {
    ...
    if (filters.visitorName && l.assignedVisitor !== filters.visitorName) return false;
  ...
  const renderRow = (item, idx) => (
    <tr key={item.id || idx} className="hover:bg-indigo-50/30 transition-colors border-b border-gray-100">
      ...
      <td className="px-4 py-3 text-center text-[13px] text-gray-600 whitespace-nowrap">{formatDisplayDate(item.leadDate)}</td>
      ...
      <td className="px-4 py-3 text-center whitespace-nowrap">
        {item.dealOutcome ? (
          <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase border ${DEAL_OUTCOME_STYLES[item.dealOutcome] || 'bg-gray-50 text-gray-600 border-gray-200'}`}>
            {item.dealOutcome}
          </span>
        ) : (
          <span className="text-gray-300">-</span>
      ...
      <td className="px-4 py-3 text-left text-[13px] text-gray-700 whitespace-nowrap max-w-[220px] truncate" title={item.rejectionReason || item.dealRemarks || ''}>
        {item.rejectionReason ? (
          <span className="text-red-700 font-medium">
            {item.rejectionReason}{item.dealRemarks ? ` (${item.dealRemarks})` : ''}
          </span>
        ) : item.dealRemarks ? (
          <span>{item.dealRemarks}</span>
        ) : (
          <span className="text-gray-300">-</span>
        )}
      </td>
      ...
      <td className="px-4 py-3 text-center text-[13px] text-gray-700 whitespace-nowrap">{item.relationshipManager || '-'}</td>
      ...
      <td className="px-4 py-3 text-center text-[13px] text-gray-500 whitespace-nowrap">{formatDisplayDate(item.followUpLoggedAt)}</td>
    </tr>
  );
    ...
    <div key={item.id} className="bg-white rounded-lg border border-indigo-50 shadow-sm p-3 space-y-2">
      <div className="flex justify-between items-start border-b border-gray-100 pb-2">
        ...
        <div className="flex flex-col items-end gap-1">
          ...
          {item.dealOutcome && (
            <span className={`text-[9px] px-2 py-0.5 rounded-full font-semibold uppercase border ${DEAL_OUTCOME_STYLES[item.dealOutcome] || 'bg-gray-50 text-gray-600 border-gray-200'}`}>
              {item.dealOutcome}
            </span>
          )}
        </div>
      ...
      <div className="grid grid-cols-2 gap-2 text-[10px]">
        ...
        {item.dealOutcome && (
          <div>
            <p className="text-gray-400 uppercase tracking-tighter text-[8px]">Deal Outcome</p>
            <p className={`font-semibold leading-tight ${item.dealOutcome === 'Closed (Won)' ? 'text-emerald-700' : 'text-red-700'}`}>{item.dealOutcome}</p>
          </div>
        )}
        {item.rejectionReason && (
          <div className="col-span-2">
            <p className="text-gray-400 uppercase tracking-tighter text-[8px]">Rejection Reason</p>
            <p className="text-red-600 font-semibold leading-tight">{item.rejectionReason}{item.dealRemarks ? ` (${item.dealRemarks})` : ''}</p>
          </div>
        )}
        <div>
        ...
        <div>
          <p className="text-gray-400 uppercase tracking-tighter text-[8px]">Logged At</p>
          <p className="text-gray-500 truncate leading-tight">{formatDisplayDate(item.followUpLoggedAt)}</p>
        </div>
      </div>
    </div>
  );
